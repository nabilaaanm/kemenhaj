...
<div class="card">
    ...
    <form action="{{ route('admin.regulasi.update', $regulation->routeParams()) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('POST')
        ...
        <div style="margin-bottom: 24px;">
            ...
            <select name="category" required
                ...
                @foreach(\App\Models\Regulasi::categoryOptions() as $value => $label)
                    <option value="{{ $value }}" {{ old('category', $regulation->category) == $value ? 'selected' : '' }}>{{ $label }}</option>
                @endforeach
            </select>
        </div>
