...
@section('title', 'Tambah Berhak Lunas')
@section('page-title', 'Tambah Berhak Lunas')
...
<div class="card">
    ...
    <h3 style="margin-bottom: 24px;">Tambah Data Berhak Lunas</h3>
    ...
    <form action="{{ route('admin.data-informasi.berhak-lunas.store') }}" method="POST">
        @csrf
        
        <div style="margin-bottom: 24px;">
            <label style="display: block; font-weight: 600; margin-bottom: 8px; color: #374151;">Nomor Porsi <span style="color: #ef4444;">*</span></label>
            <input type="text" name="nomor_porsi" value="{{ old('nomor_porsi') }}" required
        ...
        <div style="margin-bottom: 24px;">
            ...
            <select name="status" required
                ...
                <option value="Bukan Cadangan" {{ old('status') == 'Bukan Cadangan' ? 'selected' : '' }}>Bukan Cadangan</option>
            </select>
        </div>
        ...
        <div style="margin-bottom: 24px;">
            ...
            <select name="keterangan"
                ...
                <option value="Siap Berangkat" {{ old('keterangan') == 'Siap Berangkat' ? 'selected' : '' }}>Siap Berangkat</option>
                <option value="Meninggal" {{ old('keterangan') == 'Meninggal' ? 'selected' : '' }}>Meninggal</option>
                <option value="Tunda Berangkat" {{ old('keterangan') == 'Tunda Berangkat' ? 'selected' : '' }}>Tunda Berangkat</option>
                <option value="Tersambung" {{ old('keterangan') == 'Tersambung' ? 'selected' : '' }}>Tersambung</option>
                <option value="Gak Nyambung" {{ old('keterangan') == 'Gak Nyambung' ? 'selected' : '' }}>Gak Nyambung</option>
            </select>
        </div>

        <div style="margin-bottom: 24px;">
            <label style="display: block; font-weight: 600; margin-bottom: 8px; color: #374151;">KBIHU</label>
            <input type="text" name="kbihu" value="{{ old('kbihu') }}"
                   style="width: 100%; padding: 12px; border: 1px solid #d1d5db; border-radius: 8px; font-size: 14px;"
                   placeholder="KBIHU">
        </div>
