...
@section('page-title', 'Edit Infografis')
...
<div class="card">
    ...
    <h3 style="margin-bottom: 24px;">Edit Infografis Galeri</h3>
    ...
    <form action="{{ route('admin.galeri.infografis.update', $infografis->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        ...
        <div style="margin-bottom: 24px;">
            ...
            <input type="text" name="title" value="{{ old('title', $infografis->title) }}" required
        ...
        <div style="margin-bottom: 24px;">
            ...
            <textarea name="description" rows="4"
                      ...
                      placeholder="Deskripsi infografis (opsional)">{{ old('description', $infografis->description) }}</textarea>
        </div>
            ...
            <select name="category"
                ...
                @else
                    @foreach($categories as $category)
                        <option value="{{ $category->name }}" {{ old('category', $infografis->category) == $category->name ? 'selected' : '' }}>
                            {{ $category->name }}
                        </option>
                    @endforeach
                @endif
        ...
        <div style="margin-bottom: 16px;">
            <label style="display: block; font-weight: 600; margin-bottom: 8px; color: #374151;">Preview Saat Ini</label>
            <img src="{{ $infografis->image_url }}" alt="{{ $infografis->title }}"
                 style="max-width: 100%; max-height: 400px; border-radius: 8px; border: 1px solid #d1d5db;">
        </div>

        <div style="margin-bottom: 24px;">
            <label style="display: block; font-weight: 600; margin-bottom: 8px; color: #374151;">Upload File Infografis (Opsional)</label>
        ...
        <div style="margin-bottom: 24px;">
            <label style="display: block; font-weight: 600; margin-bottom: 8px; color: #374151;">Masukkan URL Infografis (Opsional)</label>
            <p style="color: #6b7280; font-size: 14px; margin-bottom: 8px;">Masukkan link gambar infografis dari internet</p>
            <input type="url" name="url" value="{{ old('url', $infografis->url) }}"
                   style="width: 100%; padding: 12px; border: 1px solid #d1d5db; border-radius: 8px; font-size: 14px;"
                   placeholder="https://example.com/infografis.png">
            <div id="urlPreview" style="margin-top: 16px; {{ old('url', $infografis->url) ? '' : 'display: none;' }}">
                <img id="urlPreviewImg" src="{{ old('url', $infografis->url) }}" alt="Preview"
                     style="max-width: 100%; max-height: 400px; border-radius: 8px; border: 1px solid #d1d5db;"
