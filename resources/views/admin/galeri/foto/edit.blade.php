...
<div class="card">
    ...
    <h3 style="margin-bottom: 24px;">Edit Foto Galeri</h3>
    ...
    <form action="{{ route('admin.galeri.foto.update', $foto->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div style="margin-bottom: 24px;">
            ...
            <input type="text" name="title" value="{{ old('title', $foto->title) }}" required
        ...
        <div style="margin-bottom: 24px;">
            ...
            <textarea name="description" rows="4"
                      ...
                      placeholder="Deskripsi foto (opsional)">{{ old('description', $foto->description) }}</textarea>
        </div>
            ...
            <select name="category"
                ...
                @else
                    @foreach($categories as $category)
                        <option value="{{ $category->name }}" {{ old('category', $foto->category) == $category->name ? 'selected' : '' }}>
                            {{ $category->name }}
                        </option>
                    @endforeach
                @endif
