...
<head>
    ...
    <title>LK & PIH - {{ $siteSetting->title_suffix }}</title>
                ...
                <div class="border border-gray-200 rounded-lg p-6 hover:shadow-md transition">
                    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                        ...
                        <div class="flex-shrink-0">
                                    @if($doc->file_url)
                                        @php
                                            $downloadName = trim((string) $doc->title);
                                            if ($downloadName !== '' && !\Illuminate\Support\Str::endsWith(strtolower($downloadName), '.pdf')) {
                                                $downloadName .= '.pdf';
                                            }
                                        @endphp
                                        <a href="{{ $doc->file_url }}" download="{{ $downloadName }}" class="inline-flex items-center gap-2 px-6 py-2.5 rounded-lg text-sm font-medium text-white hover:opacity-90 transition" style="background-color: var(--color-primary);">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
